...
void emit_segments(const RequestSpec&         r,
                   int                        request_idx,
                   int                        S,
                   std::vector<MropeSegment>& out)
{
    ...
    for (int i = 0; i < bsz; ++i) {
        ...
        for (int k = r.active_start; k < r.active_end; ++k) {
            for (int c = 0; c < 3; ++c) {
                ...
                if (got != ref) {
                    if (errors < 16) {
                        std::printf("  [%s] mismatch req=%d tok=%d c=%d  got=%d ref=%d\n",
...
int main()
{
    ...
    errors += run_case("decode_only",
                       {RequestSpec{/*seq_len=*/64, /*active_start=*/64, /*active_end=*/65,
                                    /*autoregres=*/true, /*images=*/{}}},
                       S);

    // (b) Pure-text prefill — empty images, identity positions.
    errors += run_case("pure_text",
                       {RequestSpec{32, 0, 32, false, {}}},
                       S);

    // (c) Single-image prefill (image in the middle).
    errors += run_case("single_image",
                       {RequestSpec{40, 0, 40, false,
                                    {{ /*seq_start=*/8, /*t=*/1, /*h=*/8, /*w=*/8}}}},  // 16 tokens
                       S);

    // (d) Interleaved text-image-text-image-text.
    errors += run_case("interleaved",
                       {RequestSpec{80, 0, 80, false,
                                    {{4, 1, 4, 6}, {30, 1, 6, 4}}}},
                       S);

    // (e) Video (t > 1).
    errors += run_case("video",
                       {RequestSpec{50, 0, 50, false,
                                    {{2, 2, 4, 4}}}},  // 2*2*2 = 8 tokens
                       S);
    ...
    errors += run_case("multi_image_clip",
                       {RequestSpec{80, 10, 50, false,
                                    {{6, 1, 4, 4}, {30, 2, 4, 4}}}},
                       S);
